            ...
            {booksForSelection.map(book => (
              <Card key={book.id} className={`interactive-card hover:border-primary p-1 ${selectedBook?.id === book.id ? 'border-2 border-primary shadow-primary/30' : ''}`} onClick={() => {setSelectedBook(book); setSelectedChapter(null);}}>
                <CardHeader className="p-3 text-center">
                  <Image src={book.cover_image_url || `https://placehold.co/150x200/777/FFF.png?text=${book.subject.slice(0,3)}`} alt={book.book_name} width={100} height={133} className="w-24 h-32 object-cover rounded-md mx-auto mb-2 shadow-md" data-ai-hint={book.dataAiHint || `${book.subject} textbook`}/>
